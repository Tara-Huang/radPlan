...
class RAGLung:
    def __init__(self, db: LungCancerDB, top_num=2, mode="random"):
        self.db = db
        self.top_num = top_num
    ...
    def get_ref_plans(self, patient_name):
        if self.mode == "none":
            return "No reference plan is available. Please design the optimization parameters from scratch."
        ...
        if self.mode == "random":
            shuffle(names)
            best_names = names[:self.top_num]
        
        elif self.mode == "similarity":
            # Extract features for all patients for Z-score normalization
            all_feats = []
            for n in names + [patient_name]:
                v, d = self.db.get_patient(n)
                all_feats.append(v[1:] + d[1:])
            all_feats = np.array(all_feats)
            mean = all_feats.mean(axis=0)
            std = all_feats.std(axis=0) + 1e-8

            # Query patient features
            v_q, d_q = self.db.get_patient(patient_name)
            feat_q = (np.array(v_q[1:] + d_q[1:]) - mean) / std

            similarities = []
            for name in names:
                v, d = self.db.get_patient(name)
                feat = (np.array(v[1:] + d[1:]) - mean) / std
                dist = np.linalg.norm(feat_q - feat)
                similarities.append((name, dist))

            similarities.sort(key=lambda x: x[1])
            best_names = [x[0] for x in similarities[:self.top_num]]
        else:
            best_names = names[:self.top_num]

        plans = [self.get_OptPara_DoseOut(name) for name in best_names]
        return '\n'.join(plans)
