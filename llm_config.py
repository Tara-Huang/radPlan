...
config_list = [
    {
        ...
        "base_url": "https://api.deepseek.com",  # 使用与OpenAI兼容的v1端点
        "timeout": 300,
        "max_retries": 5,
        # 把项目中所有可能被其他 Agent 调用的标签都集中贴给 DeepSeek
        "tags": ["local-gpt-4o", "gm-pro", "llama405B", "oai-gpt-4o", "local-gpt-4", "gm-flash"],
        # 若之前配置了代理才跑得通，您还可以选择保留 HTTP Client；如果不涉及被墙，可以注释掉下面这行
        #"http_client": MyHttpClient(proxy="http://localhost:7980")
    }
]
