...
class JSON2Markdown_back:
    ...
    def markdown_to_json(self, markdown_table):
        ...
        if 'Target cGy' in df.columns:
            ...
            df['Target Gy'] = df['Target Gy'].apply(
                lambda x: str(float(_safe_cell_text(x)) / 100) if _safe_cell_text(x).replace('.', '', 1).isdigit() else _safe_cell_text(x)
            )

        for _, row in df.iterrows():
            roi_name = _safe_cell_text(row['ROI Name'])
            type = _safe_cell_text(row['Objective Type'])
            target = _safe_numeric_or_text(row['Target Gy'])
            volume = _safe_numeric_or_text(row['% Volume'])
            weight = _safe_numeric_or_text(row['Weight'])
...
def _safe_numeric_or_text(value):
...
class MyOptimization(pp.Optimization):
    ...
    def create_cvxpy_problem(self):
        ...
        def _coerce_dose_value(field_name, raw_value):
            value = _safe_numeric_or_text(raw_value)
            if isinstance(value, (int, float, np.floating)):
                return float(value)
            raise ValueError(f"Invalid numeric value for {field_name}: {raw_value}")
        ...
        for i in range(len(obj_funcs)):
            if obj_funcs[i]['type'] == 'quadratic-overdose' or obj_funcs[i]['type'] == 'linear-overdose':
                if obj_funcs[i]['structure_name'] in my_plan.structures.get_structures():
                    ...
                    dose_gy_raw = self.dose_to_gy(key, obj_funcs[i][key])
                    dose_gy = _coerce_dose_value(key, dose_gy_raw) / num_fractions
        ...
        for i in range(len(constraint_def)):
            if constraint_def[i]['type'] == 'max_dose':
                ...
                if limit_key:
                    limit_raw = self.dose_to_gy(limit_key, constraint_def[i]['constraints'][limit_key])
                    limit = _coerce_dose_value(limit_key, limit_raw)
        ...
        for i in range(len(dvh_constraint)):
            if 'dose_volume' in dvh_constraint[i]['type']:
                ...
                if limit_key in dvh_constraint[i]['constraints']:
                    ...
                    dose_gy = _safe_numeric_or_text(dvh_constraint[i]['parameters']['dose_gy'])
                    if not isinstance(dose_gy, (int, float, np.floating)):
                        raise ValueError(f"Invalid numeric value for dose_gy: {dvh_constraint[i]['parameters']['dose_gy']}")
                    df_dvh_constraint.at[count, 'dose_gy'] = dose_gy
                        ...
                        if 'max_dose' in criteria[j]['type']:
                            if 'limit_dose_gy' in criteria[j]['constraints']:
                                ...
                                if org == dvh_constraint[i]['parameters']['structure_name']:
                                    ...
                                    max_dose_raw = self.dose_to_gy('limit_dose_gy', max_dose_struct)
                                    max_dose_struct = _safe_numeric_or_text(max_dose_raw)
                                    if not isinstance(max_dose_struct, (int, float, np.floating)):
                                        raise ValueError(f"Invalid numeric value for limit_dose_gy: {max_dose_raw}")
                    df_dvh_constraint.at[count, 'M'] = max_dose_struct - dose_gy
...
class MyEvaluation(pp.Evaluation):
    ...
    def __init__(self, pid, full_A=False, criteria_json_fn='./portpy_config_files/clinical_criteria/Default/Lung_2Gy_30Fx_wqx.json'):
        ...
        data_dir = r'/Users/huangyingtao/data/PortPy_datasets'
        ...
        # echo dose
        dose_file_name = os.path.join(data_dir, pid, 'DicomFiles', 'rt_dose_echo_imrt.dcm')  
...
class IMRT:
    ...
    def __init__(self, 
                 ...
                 data_dir: str = r'/Users/huangyingtao/data/PortPy_datasets',
        ...
        try:
            ...
            print(f"[Warning] Failed to load PortPy dataset: {e}")
            print("[Warning] The unit test / script might not work fully without the dataset, but an error is being suppressed for basic testing.")
            self.ct = None
            self.structs = None
            self.beams = None
            self.cc = None
    ...
    def do_optim(self, optim_param_json):
        ...
        pre_vars = None
        if self.solution and self.solution.get('optimal_intensity') is not None:
        ...
        if sol.get('optimal_intensity') is None:
            dose_md = "**Optimization Failed: Infeasible Problem**. The constraints are physically impossible to satisfy."
            dose_df = pd.DataFrame([{'Struct': 'PTV', 'Achieved Value': 0}])
